...
public class CountryArrayManager {
    ...
    public void add(CountryData countryData, int index) {
        ...
        System.arraycopy(this.countryDataArray, index, this.countryDataArray, index + 1, this.length - index);
    ...
    public void remove(int index) {
        ...
        if (index >= 0 && index < this.length) {
            System.arraycopy(this.countryDataArray, index + 1, this.countryDataArray, index, this.length - index - 1);
    ...
    public CountryData[] sortDecreasingByPopulation() {
        ...
        Arrays.sort(newArray, new Comparator<CountryData>() {
            @Override
            public int compare(CountryData left, CountryData right) {
                return Long.compare(right.getPopulation(), left.getPopulation());
            }
        });
        return newArray;
    }
    ...
    public CountryData[] filterContinent(String continent) {
        ...
        for (int i = 0; i < this.length; i++) {
            if (this.countryDataArray[i].getContinent().equals(continent)) {
                result[index++] = this.countryDataArray[i];
            }
        }
        return result;
    }
    ...
    public CountryData[] filterMostPopulousCountries(int howMany) {
        ...
        return Arrays.copyOf(sortedArray, Math.min(howMany, sortedArray.length));
    }
